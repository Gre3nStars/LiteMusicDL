...
export async function searchTracks(
  ...
  requireDesktop();
  return invoke<Track[]>("search_tracks", { query: query.trim(), sources, limit });
}

export async function downloadTrack(track: Track, directory: string): Promise<string> {
  requireDesktop();
  return invoke<string>("download_track", { track, directory });
}
...
export async function preparePlayback(track: Track): Promise<string> {
  requireDesktop();
  return invoke<string>("prepare_playback", { track });
}
